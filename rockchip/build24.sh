...
# 按工作流输入开关 Passwall 相关插件（未传值时默认启用）
if [ -z "$INCLUDE_PASSWALL" ]; then
    INCLUDE_PASSWALL="yes"
fi

if [ "$INCLUDE_PASSWALL" = "yes" ]; then
    PACKAGES="$PACKAGES luci-app-passwall luci-i18n-passwall-zh-cn"
    echo "✅ include_passwall=${INCLUDE_PASSWALL}，已添加 Passwall 相关组件"
else
    echo "ℹ️ include_passwall=${INCLUDE_PASSWALL}，跳过 Passwall 相关组件"
fi
